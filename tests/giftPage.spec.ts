...
import { modifyCartApiUrl } from '../helpers/urls'
...
test('should pass with required fields only', async ({page, baseURL}) => {
    const dataToSend = new GiftSubscriptionFormData()
        .setRecipientName(faker.person.fullName().replace(".", "").replace('\'', ''))
        .setRecipientEmail(faker.internet.email());
    await subscriptionPage.fillOutPageAndClickPay(dataToSend);
...
test('should pass with all fields', async ({page, baseURL}) => {
    ...
    const dataToSend = new GiftSubscriptionFormData()
        ...
        .setRecipientName(faker.person.fullName().replace(".", "").replace('\'', '')) //a little bug (or feature). If a recipientName contains dot, the dot will not send. 
            //The same problem with the apostrophe, but it is more serious because many surnames contain it, such as "O'Hara"
        .setRecipientEmail(faker.internet.email())
